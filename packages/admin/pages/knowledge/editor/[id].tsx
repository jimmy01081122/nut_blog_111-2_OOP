...
const Page: NextPage<IProps> = ({ id, knowledge: defaultKnowledge }) => {
  ...
  const SortableItem = SortableElement(({ value: idx }) => (
    ...
    >
      ...
      <Popconfirm title="確認刪除?" onConfirm={() => deleteKnowledge(idx)} okText="確定" cancelText="取消">
  ...
  const goback = useCallback(() => {
    ...
    // ignore-me
    const newErr = new Error('請完成操作後關閉頁面');
    throw newErr;
  }, [save]);
    ...
    <div className={styles.wrapper}>
      ...
      <main>
        {currentChapter ? (
          ...
          <div className={styles.helper}>請新建章節（或者選擇章節進行編輯）</div>
        )}
      </main>
    </div>
  );
